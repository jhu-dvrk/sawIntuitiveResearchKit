...
void mtsTeleOperationPSM::RunJawGripperTeleop() {
    ...
    if (!m_jaw_caught_up_after_clutch) {
        if (gripper_error < mtsIntuitiveResearchKit::TeleOperationPSM::ToleranceBackFromClutch) {
            m_jaw_caught_up_after_clutch = true;
        }
    }

    // pick the jaw speed based on back from clutch or not
    const double jaw_rate = m_jaw_caught_up_after_clutch ? m_jaw.rate : m_jaw.rate_back_from_clutch;
